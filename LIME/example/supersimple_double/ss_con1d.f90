...
!*****************************************************************************************
  ...
  REAL*8    :: W      ! total width of 1-D domain
  REAL*8    :: dx     ! width of finite difference discretization
  REAL*8    :: k      ! thermal conductivity
  REAL*8    :: rhoCp  ! density*specific heat
  ! Boundary Condition related -------------------------------------------------
  REAL*8    :: qlt    ! specified heat flux at the left-side boundary
  REAL*8    :: qrt    ! computed heat flux at the right-side boundary
  REAL*8    :: h_c    ! convective heat trasnfer coeficient at the left-side boundary
  REAL*8    :: Tf     ! fluid temperature "seen" by the left-side boundary
  REAL*8    :: Tr     ! far-field radiation temperature "seen" by the right-side boundary
  ! State variables and/or source term related ---------------------------------
  REAL*8    :: time   ! time (sec)
  REAL*8    :: tmax   ! maximum time 
  REAL*8    :: dt     ! time step
  REAL*8    :: Qs(3)  ! heat source array
  REAL*8    :: T(3)   ! temperature array during solution iteration
  REAL*8    :: T0(3)  ! temperature array (converged) at previous time step
  ! Solver and Solution --------------------------------------------------------
  REAL*8    :: R(3)   ! Residual array 
  REAL*8    :: R_max  ! maximum value in the Residual array
  ...
  REAL*8    :: ebal   ! normalized energy balance  (=1.0 at steady state)
  REAL*8    :: f      ! relaxation factor for SS iteration method
...
!*****************************************************************************************
  USE con1d_mod
  IMPLICIT NONE
  REAL*8 :: Qbar  ! volume weighted average heat source
...
!*****************************************************************************************
  ...
  REAL*8 :: J(3,3)  ! Jacobian matrix.  Note  J(eq_index, T_index)
