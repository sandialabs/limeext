...
!*****************************************************************************************
  ...
  REAL*8    :: rhoCpW ! density*specific heat*width of wall
  REAL*8    :: Tfluid ! Fluid Temperature on the right-side boundary
  REAL*8    :: hconv  ! Convective heat trasnver coeficient on the right-side boundary
  REAL*8    :: Trad   ! Radiation Temperature seen by the left-side boundary
  REAL*8    :: time   ! time (sec)
  REAL*8    :: dt     ! time step
  REAL*8    :: T(1)   ! temperature (lumped mass approximation) of the wall
  REAL*8    :: T0     ! temperature at previous time step
  REAL*8    :: R(1)   ! Residual array 
  REAL*8    :: dRdT   ! derivative of Residual wrt Temperature 
  REAL*8    :: delT   ! Newton-based temperature update
  REAL*8    :: coef1  ! coeficient used in energy equation
  REAL*8    :: coef2  ! coeficient used in energy equation
  REAL*8    :: coef3  ! coeficient used in energy equation
  REAL*8    :: tol    ! convergence criteria tolerance
  REAL*8    :: qrad   ! radiation heat trasnfer on the left side (per unit area)
  REAL*8    :: qfluid ! convective heat transfer on the right side (per unit area)
...
!*****************************************************************************************
  ...
  uout   = 11       ! standard output goes to unit 6
...
!*****************************************************************************************
  IMPLICIT NONE
  REAL*8,INTENT(in)  :: T      ! temperature (lumped mass approximation) of the wall
  REAL*8,INTENT(in)  :: T0     ! temperature at previous time step
  REAL*8,INTENT(in)  :: Trad   ! Radiation Temperature seen by the left-side boundary
  REAL*8,INTENT(in)  :: Tfluid ! Fluid Temperature on the right-side boundary
  REAL*8,INTENT(in)  :: coef1  ! coeficient used in energy equation
  REAL*8,INTENT(in)  :: coef2  ! coeficient used in energy equation
  REAL*8,INTENT(in)  :: coef3  ! coeficient used in energy equation
  REAL*8,INTENT(out) :: R(1)   ! Residual 
! --------------------------------------------------------------------
