...
!*****************************************************************************************
  ...
  IMPLICIT NONE
     ...
     DO iter = 1,15
        ...
        CALL residual2_thinwall(T,T0,Trad,Tfluid,coef1,coef2,coef3, R)
        ...
        ! write(uout,'(a,i3,a,3es11.3)') "DEBUG: iter=", iter, "  Res, delT, T+delT= ",R, delT, T+delT

        ! update to temperature estimate
        T = T + delT
     END DO
     ! ------------------------------------------------------------------------
     IF(abs(R) .gt. tol) write(uout,*) "Warning: ThinWall Iteration did not converge!"
...
!*****************************************************************************************
  ...
  IMPLICIT NONE
     ...
     write(uout,9) time, qrad,T,qfluid,iter, abs(R)
