...
// using the following Model Evaluators: CON1D_ModelEval_w_Resid, and NEUTRON_ModelEval
// 
// ********************************************************************************************
//
...
int main(int argc, char *argv[]) {
  ...
  try {    // the "try" construct is used to allow for catching "exceptions" that might
    ...
    //    (on the heap)
    //    This is the same setup as the simpler example involving CON1D_ModelEval but 
    //    demonstrates enhanced coupling capabilities enabled by exposing the residual
    //    vector within the ss_con1d.f90 module.
    ...
    //    the Problem manager
          ...
          // Optional way to directly set max_fixed_point_iterations and fixed_point_tolerance
          // These over-ride default values and/or values set in input file Problem_Manager_setup_fp.xml
          pm.set_max_fixed_point_iters(50); // Allow sufficient iterations
          pm.set_fixed_point_tolerance(1.e-3);

          // optional output routine not used right now
